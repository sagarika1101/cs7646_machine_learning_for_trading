...
class DTLearner(object):
    ...
    def build_tree(self,dataX,dataY):
    	...
    	if result:
    		...
    		#Xi having highest absolute value correlation with Y
    		highest_corr = -1  #initialize max value
	    	best_feature = None

	    	for i in range(0,dataX.shape[1]):  #for all features
	    		corr_val = np.corrcoef(dataX[:,i],dataY)
	    		corr_val = abs(corr_val[0,1])   #absolute value correlation
