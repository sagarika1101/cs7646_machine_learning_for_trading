...
def testPolicy(symbol = "AAPL", sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2011,12,31), sv = 100000):
	...
	# for i in range(trades_df.shape[0]):
	# 	#Check if SMA<0.95 or momentum<-0.5
	# 	if (SMA.iloc[i,0] < 0.95 or momentum.iloc[i,0] < -0.5):
	...
	# 			trades_df.iloc[i,2] = 2000.0
	# 	#Check if SMA>1.05 or momentum>0.5
	# 	elif (SMA.iloc[i,0] > 1.05 or momentum.iloc[i,0] > 0.5):
	# 		if (current_holdings==0.0):
	# 			trades_df.iloc[i,1] = 'SELL'
	# 			current_holdings-=1000.0
	# 			trades_df.iloc[i,2] = 1000.0
	# 		if current_holdings==1000.0:
	# 			trades_df.iloc[i,1] = 'SELL'
	# 			current_holdings-=2000.0
	# 			trades_df.iloc[i,2] = 2000.0
